...
impl AppState {
    ...
    fn tick(&mut self) {
        ...
        if !self.paused || step_triggered {
            ...
            let (next_grid, alive) = update_grid(
                &self.grid,
                self.w_sim,
                self.h_sim,
                &self.birth_rules,
                &self.survive_rules,
                &self.rules_neighbor_mask,
            );
            self.grid = next_grid;
            self.current_alive = alive;
            self.generation += 1;
    ...
    let total_alive = new_grid
        ...
        .map(|(y, row)| {
            ...
            for x in 0..w {
                ...
                let mut n = 0;
                if mask[0] && grid[ym + xm] {
                    n += 1;
                }
                if mask[1] && grid[ym + x] {
                    n += 1;
                }
                if mask[2] && grid[ym + xp] {
                    n += 1;
                }
                if mask[3] && grid[y_offset + xm] {
                    n += 1;
                }
                if mask[4] && grid[y_offset + xp] {
                    n += 1;
                }
                if mask[5] && grid[yp + xm] {
                    n += 1;
                }
                if mask[6] && grid[yp + x] {
                    n += 1;
                }
                if mask[7] && grid[yp + xp] {
                    n += 1;
                }
...
fn get_braille_char(grid: &[bool], w: usize, h: usize, char_col: usize, char_row: usize) -> char {
    ...
    unsafe {
        if base_y + 3 < h && base_x + 1 < w {
            ...
            if *grid.get_unchecked(y0_offset + base_x) {
                val |= 0x01;
            }
            if *grid.get_unchecked(y0_offset + base_x + 1) {
                val |= 0x08;
            }
            if *grid.get_unchecked(y1_offset + base_x) {
                val |= 0x02;
            }
            if *grid.get_unchecked(y1_offset + base_x + 1) {
                val |= 0x10;
            }
            if *grid.get_unchecked(y2_offset + base_x) {
                val |= 0x04;
            }
            if *grid.get_unchecked(y2_offset + base_x + 1) {
                val |= 0x20;
            }
            if *grid.get_unchecked(y3_offset + base_x) {
                val |= 0x40;
            }
            if *grid.get_unchecked(y3_offset + base_x + 1) {
                val |= 0x80;
            }
        } else {
            // Slow path (fall back to safe bounds checked accesses at the very edges of the grid)
            if base_y < h {
                if base_x < w && *grid.get_unchecked(y0_offset + base_x) {
                    val |= 0x01;
                }
                if base_x + 1 < w && *grid.get_unchecked(y0_offset + base_x + 1) {
                    val |= 0x08;
                }
            }
            if base_y + 1 < h {
                if base_x < w && *grid.get_unchecked(y1_offset + base_x) {
                    val |= 0x02;
                }
                if base_x + 1 < w && *grid.get_unchecked(y1_offset + base_x + 1) {
                    val |= 0x10;
                }
            }
            if base_y + 2 < h {
                if base_x < w && *grid.get_unchecked(y2_offset + base_x) {
                    val |= 0x04;
                }
                if base_x + 1 < w && *grid.get_unchecked(y2_offset + base_x + 1) {
                    val |= 0x20;
                }
            }
            if base_y + 3 < h {
                if base_x < w && *grid.get_unchecked(y3_offset + base_x) {
                    val |= 0x40;
                }
                if base_x + 1 < w && *grid.get_unchecked(y3_offset + base_x + 1) {
                    val |= 0x80;
                }
            }
        }
...
fn render_dashboard(
    ...
    let format_kv =
        |label: &'static str, val_styled: Span<'static>, val_raw: &str| -> Line<'static> {
            let vis_label = label.len();
            let vis_val = val_raw.len();
            let spaces = w_char
                .saturating_sub(2)
                .saturating_sub(vis_label)
                .saturating_sub(vis_val);
    ...
    // 2. Stats
    let mode_str = if stats.mode == GridMode::Braille {
        "Braille (Hi-Res)"
    } else {
        "Square"
    };
    ...
    lines.push(pad_spans(
        format_kv(" Grid Mode:", mode_val, mode_str).spans,
    ));
    ...
    lines.push(pad_spans(
        format_kv(" Theme:", theme_val, theme_name_short).spans,
    ));
    ...
    let rules_val = Span::styled(
        rules_str.clone(),
        Style::default().fg(Color::Rgb(203, 166, 247)),
    ); // mauve
    lines.push(pad_spans(
        format_kv(" Ruleset:", rules_val, &rules_str).spans,
    ));
    ...
    let alive_spaces = w_char
        .saturating_sub(2)
        .saturating_sub(" Alive Cells:".len())
        .saturating_sub(alive_raw.len())
        .max(1);
    lines.push(pad_spans(vec![
        Span::styled(
            " Alive Cells:",
    ...
    let (growth_span, growth_raw) = if stats.growth > 0 {
        ...
        (
            Span::styled("0", Style::default().fg(Color::Rgb(166, 173, 200))),
            "0".to_string(),
        )
    };
    lines.push(pad_spans(
        format_kv(" Growth Rate:", growth_span, &growth_raw).spans,
    ));

    let status_str = if stats.paused {
        "Paused (Step)"
    } else {
        "Running"
    };
    ...
    lines.push(pad_spans(
        format_kv(" Status:", status_val, status_str).spans,
    ));

    let gmode_str = if stats.graph_mode == GraphMode::Full {
        "Full History"
    } else {
        "Rolling"
    };
    ...
    lines.push(pad_spans(
        format_kv(" Graph Mode:", gmode_val, gmode_str).spans,
    ));

    lines.push(pad_spans(vec![Span::styled(
        "-".repeat(w_char),
        Style::default().fg(Color::Rgb(88, 91, 112)),
    )]));

    // 3. Braille Live Plot
    let g_title = if stats.graph_mode == GraphMode::Full {
        "📈 Live Pop. (Full)"
    } else {
        "📈 Live Pop. (Rolling)"
    };
    ...
    if history.len() >= 2 {
        ...
        for r in 0..graph_h_char {
            ...
            for c in 0..graph_w_char {
                ...
                if canvas[base_y * pw + base_x] != 0 {
                    val |= 0x01;
                }
                if canvas[base_y * pw + base_x + 1] != 0 {
                    val |= 0x08;
                }
                if canvas[(base_y + 1) * pw + base_x] != 0 {
                    val |= 0x02;
                }
                if canvas[(base_y + 1) * pw + base_x + 1] != 0 {
                    val |= 0x10;
                }
                if canvas[(base_y + 2) * pw + base_x] != 0 {
                    val |= 0x04;
                }
                if canvas[(base_y + 2) * pw + base_x + 1] != 0 {
                    val |= 0x20;
                }
                if canvas[(base_y + 3) * pw + base_x] != 0 {
                    val |= 0x40;
                }
                if canvas[(base_y + 3) * pw + base_x + 1] != 0 {
                    val |= 0x80;
                }
        ...
        let spaces_count = graph_w_char
            .saturating_sub(left_lbl.len())
            .saturating_sub(right_lbl.len());
        lines.push(pad_spans(vec![
            Span::raw(" ".repeat(7)),
    ...
    let make_shortcut_line = |k1: &'static str,
                              l1: &'static str,
                              k2: &'static str,
                              l2: &'static str,
                              k3: &'static str,
    ...
    lines.push(pad_spans(make_shortcut_line(
        "Space", "Pause", "N", "Step", "D", "Dash",
    )));
    lines.push(pad_spans(make_shortcut_line(
        "M", "Grid M", "V", "Graph", "S", "Style",
    )));
    lines.push(pad_spans(make_shortcut_line(
        "R", "Random", "C", "Clear", "Q", "Quit",
    )));
    lines.push(pad_spans(make_shortcut_line(
        "E", "Rules", "T", "Theme", "H", "Help",
    )));
...
fn render_help_popup(f: &mut Frame, area: ratatui::layout::Rect, state: &AppState) {
    ...
    let text = vec![
        ...
        Line::from(format!(
            "     ihrer Nachbarn in S ({}) enthalten ist.",
            if survive_str.is_empty() {
                "keine"
            } else {
                &survive_str
            }
        ...
        Line::from(format!(
            "     ihrer Nachbarn in B ({}) enthalten ist.",
...
fn render_rules_popup(f: &mut Frame, area: ratatui::layout::Rect, state: &AppState) {
    ...
    for (i, preset) in PRESET_RULESETS.iter().enumerate() {
    ...
    for col in 0..9 {
        ...
        let is_focused =
            matrix_focused && state.rules_matrix_row == 0 && state.rules_matrix_col == col;
    ...
    for col in 0..9 {
        ...
        let is_focused =
            matrix_focused && state.rules_matrix_row == 1 && state.rules_matrix_col == col;
    ...
    lines.push(Line::from(vec![
        ...
        make_mask_box(3, "←"),
        Span::raw(" "),
        Span::styled(" [C] ", Style::default().fg(Color::Rgb(203, 166, 247))),
        Span::raw(" "),
        make_mask_box(4, "→"),
    ]));
    lines.push(Line::from(vec![
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(
        "  [Tab] Bereich wechseln | [Pfeiltasten] Navigieren / Slider regeln",
    ));
    lines.push(Line::from(
        "  [Space] Togglen | [R] Spielfeld randomisieren | [E/Esc] Schließen",
    ));
...
fn render_theme_popup(f: &mut Frame, area: ratatui::layout::Rect, state: &AppState) {
    ...
    for (i, style_cfg) in STYLES.iter().enumerate() {
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(
        "RGB Custom Farbeditor (Pfeiltasten links/rechts zum Ändern):",
    ));
    lines.push(Line::from(""));
    ...
    let box_str = if state.use_custom_colors {
        "[x]"
    } else {
        "[ ]"
    };
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!(
                "{} Eigene Farben verwenden (Checkbox mit Space toggeln)",
                box_str
            ),
            cb_style,
        ),
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(
        "  [Tab/Pfeiltasten] Navigieren | [Pfeil Links/Rechts] Slider regeln",
    ));
    lines.push(Line::from(
        "  [Space] Togglen / Reset auf Preset | [T/Esc] Schließen",
    ));
    let paragraph = Paragraph::new(lines)
        .block(block)
        .style(Style::default().fg(Color::Rgb(205, 214, 244)));
...
fn make_status_text(
    ...
    if show_dashboard {
        let text = format!(
            " Pause: {} | Gen: {} | Alive: {} | Speed: tgt:{} act:{} fps",
...
fn draw_ui(f: &mut Frame, state: &mut AppState) {
    ...
    let (horizontal_chunks, current_dash_w) = {
        ...
        let chunks = if current_dash_w > 0 {
            let mut grid_w = main_area
                .width
                .saturating_sub(current_dash_w)
                .saturating_sub(1);
            ...
            Layout::default()
                .direction(Direction::Horizontal)
                .constraints([Constraint::Length(grid_w), Constraint::Length(remaining_w)])
                .split(main_area)
        };
    ...
    let (active_cell_color, active_bg_color) = if state.use_custom_colors {
        (
            Color::Rgb(
                state.theme_custom_r,
                state.theme_custom_g,
                state.theme_custom_b,
            ),
            Color::Rgb(
                state.theme_custom_bg_r,
                state.theme_custom_bg_g,
                state.theme_custom_bg_b,
            ),
        )
    } else {
        ...
        for r in 0..h_render {
            ...
            for c in 0..w_render {
                ...
                let (ch, cell_style) = if active_style.is_solid {
                    ...
                    let text = if cell {
                        active_style.alive_str
                    } else {
                        active_style.dead_str
                    };
                    let fg = if cell {
                        active_cell_color
                    } else {
    ...
    loop {
            ...
            && key.kind == KeyEventKind::Press
        {
            if state.popup != ActivePopup::None {
                match state.popup {
                    ActivePopup::None => {}
                    ActivePopup::Help => match key.code {
                        KeyCode::Char('h')
                        | KeyCode::Char('H')
                        | KeyCode::Char('?')
                        | KeyCode::Esc => {
                    ...
                    ActivePopup::RulesEditor => match key.code {
                        ...
                        KeyCode::Up => match state.rules_focus {
                        ...
                        KeyCode::Down => match state.rules_focus {
                            RulesFocus::PresetList => {
                        ...
                        KeyCode::Left => match state.rules_focus {
                            RulesFocus::PresetList => {}
                            RulesFocus::TotalisticMatrix => {
                        ...
                        KeyCode::Right => match state.rules_focus {
                            RulesFocus::PresetList => {}
                            RulesFocus::TotalisticMatrix => {
                        ...
                        KeyCode::Char(' ') => match state.rules_focus {
                            RulesFocus::PresetList => {
                            ...
                            RulesFocus::TotalisticMatrix => {
                                if state.rules_matrix_row == 0 {
                                    state.rules_custom_birth[state.rules_matrix_col] =
                                        !state.rules_custom_birth[state.rules_matrix_col];
                                } else {
                                    state.rules_custom_survive[state.rules_matrix_col] =
                                        !state.rules_custom_survive[state.rules_matrix_col];
                                }
                            ...
                            RulesFocus::NeighborhoodMask => {
                                state.rules_neighbor_mask[state.rules_mask_idx] =
                                    !state.rules_neighbor_mask[state.rules_mask_idx];
                            }
                            RulesFocus::DensitySlider => {}
                        },
                    ...
                    ActivePopup::ThemeManager => match key.code {
                        ...
                        KeyCode::Up => match state.theme_focus {
                        ...
                        KeyCode::Down => match state.theme_focus {
                            ThemeFocus::PresetList => {
                                if state.style_idx + 1 < STYLES.len() {
                                    ...
                                    if !state.use_custom_colors {
                                        ...
                                        let (fg_r, fg_g, fg_b) = color_to_rgb(active_preset.color);
                                        let (bg_r, bg_g, bg_b) =
                                            color_to_rgb(active_preset.bg_color);
                                        state.theme_custom_r = fg_r;
                                        state.theme_custom_g = fg_g;
                                        state.theme_custom_b = fg_b;
                                        state.theme_custom_bg_r = bg_r;
                            ...
                            ThemeFocus::CustomCheckbox => {
                                state.theme_focus = ThemeFocus::PresetList
                            }
                        },
                        KeyCode::Left => match state.theme_focus {
                            ThemeFocus::FgRed => {
                                state.theme_custom_r = state.theme_custom_r.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::FgGreen => {
                                state.theme_custom_g = state.theme_custom_g.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::FgBlue => {
                                state.theme_custom_b = state.theme_custom_b.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgRed => {
                                state.theme_custom_bg_r = state.theme_custom_bg_r.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgGreen => {
                                state.theme_custom_bg_g = state.theme_custom_bg_g.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgBlue => {
                                state.theme_custom_bg_b = state.theme_custom_bg_b.saturating_sub(5);
                                state.use_custom_colors = true;
                            }
                            _ => {}
                        },
                        KeyCode::Right => match state.theme_focus {
                            ThemeFocus::FgRed => {
                                state.theme_custom_r = state.theme_custom_r.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::FgGreen => {
                                state.theme_custom_g = state.theme_custom_g.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::FgBlue => {
                                state.theme_custom_b = state.theme_custom_b.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgRed => {
                                state.theme_custom_bg_r = state.theme_custom_bg_r.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgGreen => {
                                state.theme_custom_bg_g = state.theme_custom_bg_g.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            ThemeFocus::BgBlue => {
                                state.theme_custom_bg_b = state.theme_custom_bg_b.saturating_add(5);
                                state.use_custom_colors = true;
                            }
                            _ => {}
                        },
                        KeyCode::Char(' ') => match state.theme_focus {
                            ThemeFocus::CustomCheckbox => {
